...
function addTwoToArray(numbers , fn){
  let final = [];
  for(let i = 0 ; i <= numbers.length ; i++){
    final.push(fn(numbers[i]))
  }
  return final;
}
console.log(addTwoToArray([1, 2, 3], addTwo));
...
function addTwoToArray(numbers , fn){
  ...
  for(let i = 0 ; i < numbers.length ; i++){
    ...
    let adding = fn(numbers[i]);
    final.push(adding);
  }
  return final;
}
addTwoToArray([1,2,3] , addTwo);
...
function  addThreeToArray (numbers ,fn){
  let final =  [];
...
addThreeToArray([1,2,3] , addThree);
...
function  addFiveToArray (numbers ,fn){
  let final =  [];
  for(let num of numbers){
    let add = fn(num);
    final.push(add);
...
addThreeToArray([1,2,3,4,5] , addFive);
...
*/
function changeArray (number , callback){
  let newOne = [];
  for(let numOn of number){
    console.log(numOn)
    let jj = callback(numOn);
    newOne.push(jj);
  }
  return newOne
}
changeArray([1,2,3,4], addTwo);
changeArray([1,2,3,4], addThree);
changeArray([1,2,3,4], addFive);
/*
...
function sendMessage (message , call){
  return call(message);
}
console.log(sendMessage(`hello ` , console.log));
console.log(sendMessage("Hello Alert!", alert));
...
function filterMale(arr , fn){
  return (fn(arr));
}
filterAdult(people , isMale);
/*
...
function filterFemale(arr , fn){
  return (fn(arr));
}
filterAdult(people , isFemale);
/*
